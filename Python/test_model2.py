...
currentDatetime = datetime(2018, 3, 15, 9)
print("Current datetime: " + str(currentDatetime))
...
#print(str(test_data))
model_in = open("finalised_mlp.pkl", "rb")
...
jsonString += "  [\n";
columnNames = ['Date', 'Label'];
numberOfJSONObjects = len(dateVector);
for index in range(0, numberOfJSONObjects):
  jsonString += "    {\"" + columnNames[0] + "\": \"" + str(dateVector[index]) + "\", \"" + columnNames[1] +  "\": " + str(prediction[index]) + "}";
...
try:
  ...
  print('Querying ' + dbdatabase + ' on Predictions...')
  cursor = conn.cursor()
  for index in range(len(prediction)):
