...
void string_copy(char *to, char *from) {
    ...
    while ((*to++ = *from++) != '\0')
        length++;
    printf("string_copy (lengthFrom): %d\n\n", length);
}

void string_concatenate(char *to, char *from) {
    //int lengthTo = sizeof(to) / sizeof(char);
    //printf("string_concatenate (lengthTo): %d\n", lengthTo);
    //int lengthFrom = sizeof(from) / sizeof(char);
    //printf("string_concatenate (lengthFrom): %d\n\n", lengthFrom);
    ...
    printf("To: %s\n", to);
    printf("From: %s\n", from);
    char *toTemp = (char *) malloc(sizeof(char) * 20);
    string_copy(toTemp, to);

    char *fromTemp = (char *) malloc(sizeof(char) * 20);
    string_copy(fromTemp, from);
    ...
    int lengthNewTo = lengthTo + lengthFrom + 1;
    char *newTo = (char *) malloc (sizeof(char) * lengthNewTo);

    printf("Before From\n");
    printf("First Element Content of from: %s\n", from);
    string_copy(newTo, to);
    string_copy(newTo + lengthTo, from);
    printf("NewTo: %s\n", newTo);

    int checkLengthNewTo = 0;
    while (*newTo++ != '\0')
        checkLengthNewTo++;

    string_copy(to, newTo);

    printf("Length to: %d\n", lengthTo);
    printf("Length from: %d\n", lengthFrom);
    printf("Length newTo: %d\n", lengthNewTo);
    printf("Check length newTo: %d\n\n", checkLengthNewTo);
}
        ...
        case 3: cout << "Observatory Data..." << endl;
                ...
                if (infile) {
                  ...
                  while (fgets(line, sizeof(line), infile)) {
                    words = strtok(line, ", ");
                    printf("Old Date: %s \n", words);
                    changeDateFormat(words);
                    //strcpy(observatoryData[numar].date, temp);
